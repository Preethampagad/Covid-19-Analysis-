...
--SELECT location,SUM(CAST(icu_patients AS INT))AS total_icu_patients, SUM(CAST(hosp_patients AS INT)) AS total_hospital_admits
--FROM  sqlprojects.dbo.CovidDeaths
--WHERE continent IS NOT NULL
--GROUP BY location
--ORDER BY location

SELECT Continent, location,population,MAX(new_vaccinations) AS total_vaccinated, (MAX(new_vaccinations)/population)*100 as percentage_of_pop_vaccinated
FROM #Percentage_of_people_Vaccinated
GROUP BY Continent,location,population
